...
def _db_error(resp, default_msg: str = "Database error"):
    """Raise HTTP 500 with the best available DB error message."""
    msg = getattr(resp, "error", None)
    detail = getattr(msg, "message", None) if msg else default_msg
    raise HTTPException(status_code=500, detail=detail)


def _get_user_role_and_hospital(user_id: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Return (role, hospital_id) for the given user_id.
    - Prefer Admin hospital if present; otherwise Nurse hospital.
    - Role is read from User.role (falls back to 'admin'/'nurse' when inferred).
    """
    ...
    # Not found in either — still try to read role from User for clarity
    role_r = (
        supabase.table(USER_TABLE)
        .select("role")
        .eq("user_id", user_id)
...
def _assert_accident_managed_by_user(accident_id: str, user_id: str) -> Dict[str, Any]:
    """
    Ensure the accident is currently managed by user_id; return minimal row.
    """
    supabase = get_supabase()
    r = (
    ...
    if r.data is None:
        if getattr(r, "error", None):
            _db_error(r, "Failed to load accident record")
...
def _ensure_same_hospital(user_hospital_id: Optional[str], hospital_id: Optional[str], message: str) -> None:
    """Raise 403 if hospital_id does not match user_hospital_id."""
...
def create_transfer_request_service(
    *,
    ...
    Nurse requests a transfer:
      ...
      - from_hospital = nurse's hospital
    ...
    if resp.data is None or len(resp.data) == 0:
        _db_error(resp, "Failed to create transfer request.")
    return resp.data[0]
...
def list_my_outgoing_transfers_service(user: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Nurse: list transfers created from their hospital (outgoing).
    """
    supabase = get_supabase()
    user_id = user.get("sub")
    if not user_id:
    ...
    if r.data is None and getattr(r, "error", None):
        _db_error(r)
    return r.data or []
...
def list_incoming_transfers_for_admin_service(user: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Admin: list pending transfers where to_hospital = admin's hospital and approved_by IS NULL.
    """
    supabase = get_supabase()
    user_id = user.get("sub")
    if not user_id:
...
def approve_transfer_service(
    *,
    ...
    Returns the updated Transfer row (or reloads it if the RPC returns nothing).
    """
    supabase = get_supabase()
    ...
    if t.data is None:
        if getattr(t, "error", None):
            _db_error(t, "Failed to load transfer")
    ...
    if not rpc.data:
        # Reload if RPC doesn't return the row
        refreshed = (
            supabase.table(TRANSFER_TABLE)
            .select("*")
            .eq("transfer_id", transfer_id)
        ...
        if refreshed.data is None:
            if getattr(refreshed, "error", None):
                _db_error(refreshed, "Approval succeeded but failed to load transfer.")
...
def reject_transfer_service(
    *,
    ...
    Admin rejects a transfer:
      ...
      - Deletes the Transfer row (no change to Accident Record or Hospital_Patient)
    """
    supabase = get_supabase()
    admin_id = user.get("sub")
    if not admin_id:
    ...
    if getattr(d, "error", None):
        _db_error(d, "Failed to reject (delete) transfer.")

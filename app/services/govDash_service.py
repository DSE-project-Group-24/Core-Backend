...
def _get_data_period(accident_data: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    if dates:
        try:
            ...
            # If min/max fails (e.g., incomparable types), fall back to None
            return None
...
def get_accident_summary_service() -> Dict[str, Any]:
    ...
    # Get all accident data (no filters) - paginated
    all_data = []
    page_size = 1000
    start = 0
    
    while True:
        response = supabase.table("Accident Record").select("*").range(start, start + page_size - 1).execute()
...
def get_filter_options_service() -> Dict[str, Any]:
    ...
    # Get all accident data with patient info to extract unique values (paginated)
    all_data = []
    page_size = 1000
    start = 0
...
def _get_filtered_accident_data(supabase, filters: AccidentAnalyticsFilters) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        while True:
            query = supabase.table("Accident Record").select(select_str, count='exact')
...
#     supabase = get_supabase()
#     # Select both date and severity
#     resp = supabase.table("Accident Record").select('"incident at date","Severity"').execute()
